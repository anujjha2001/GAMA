    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-[32px] glass-panel p-6 border border-border space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Theme Mode</h3>
            ...
            <div className="grid grid-cols-2 gap-3 p-1 bg-muted rounded-2xl">
              <button
                ...
                className={`py-3 rounded-xl flex flex-col items-center justify-center gap-2 transition-all cursor-pointer ${
                  theme === 'light' 
                    ? 'bg-card text-foreground shadow-md font-semibold' 
            ...
            <div className="border-t border-border pt-6 space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Wearable Simulation</h3>
          ...
          <div className="rounded-[32px] glass-panel p-6 border border-border space-y-6">
            <div className="flex justify-between items-center">
              <div className="space-y-1">
                <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">AURA Long-Term Health Memory</h3>
                <p className="text-[10px] text-muted-foreground">
                  Persistent facts, meal choices, preferences, and conditions AURA cross-references during conversations.
                </p>
              </div>
            ...
            {/* Add Memory Form */}
            <div className="p-4 bg-muted rounded-[20px] border border-border flex flex-col md:flex-row gap-3">
              <div className="w-full md:w-1/3">
                <select
                  value={newTagCat}
                  onChange={(e: any) => setNewTagCat(e.target.value)}
                  className="w-full bg-card border border-border rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-violet-500"
                >
              ...
              <div className="flex-1 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-card border border-border rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-violet-500"
                  onKeyDown={(e) => {
              ...
              <AnimatePresence>
                {memoryTags.map((tag) => (
                  <motion.div
                    ...
                    className="flex justify-between items-center p-3.5 bg-card/40 border border-border/60 hover:border-border rounded-2xl transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider ${
                        tag.category === 'medical' 
                    ...
                    <button
                      ...
                      className="text-muted-foreground hover:text-rose-500 p-1 rounded-lg hover:bg-muted transition-colors cursor-pointer"
                    >
