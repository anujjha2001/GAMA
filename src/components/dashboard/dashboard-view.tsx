...
export function DashboardView() {
  ...
  const {
    ...
    memoryTags,
    dashboardBackgroundType, dashboardBackgroundUrl, dashboardBackgroundOverlay, dashboardPlaybackOptions,
    setDashboardBackground, setDashboardOverlay, setDashboardPlayback
  } = useHealthStore();
    ...
    async function loadBackgroundSettings() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.settings) {
            if (data.settings.dashboardBackgroundType) {
               setDashboardBackground(data.settings.dashboardBackgroundType, data.settings.dashboardBackgroundUrl);
            }
            if (data.settings.dashboardBackgroundOverlay) {
               setDashboardOverlay(data.settings.dashboardBackgroundOverlay);
            }
            if (data.settings.dashboardPlaybackOptions) {
               setDashboardPlayback(data.settings.dashboardPlaybackOptions);
            }
          }
        }
      } catch (e) { console.error('Failed to load bg settings', e); }
    }
  ...
  return (
    <div ref={containerRef} className="flex flex-col gap-12 pb-32 w-full relative min-h-screen font-sans text-[#eae3dc]">
      ...
      <div className="fixed inset-0 pointer-events-none -z-20 overflow-hidden bg-[#070709]">
        ...
        >
          {dashboardBackgroundType === 'video' || dashboardBackgroundUrl?.endsWith('.mp4') ? (
            <video
              ...
              autoPlay={dashboardPlaybackOptions?.autoplay ?? true}
              loop={dashboardPlaybackOptions?.loop ?? true}
              muted={dashboardPlaybackOptions?.muted ?? true}
              playsInline
            ...
            />
          ) : dashboardBackgroundType === 'motion' ? (
             <div className="w-full h-full bg-gradient-to-br from-indigo-500/20 via-purple-500/20 to-emerald-500/20 animate-pulse" 
               style={{
                 filter: `brightness(${dashboardBackgroundOverlay?.brightness ?? 110}%) contrast(${dashboardBackgroundOverlay?.contrast ?? 110}%) saturate(${dashboardBackgroundOverlay?.saturation ?? 125}%) blur(${dashboardBackgroundOverlay?.blur ?? 0}px)`,
               }}
             />
        ...
        <div className="absolute top-[-20%] left-[20%] w-[600px] h-[600px] rounded-full bg-blue-500/10 blur-[120px] pointer-events-none z-10" />
        <div className="absolute bottom-[-10%] right-[10%] w-[500px] h-[500px] rounded-full bg-emerald-500/10 blur-[120px] pointer-events-none z-10" />
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#070709] via-[#070709]/80 to-[#070709]/40 z-20" />
      </div>
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-neutral-400 max-w-xl leading-relaxed mb-8">
            Your centralized nervous system for health optimization, training intelligence, and cognitive enhancement.
          </p>

          {/* AURA Interactive Input Bar */}
          <div className="w-full max-w-2xl text-left relative z-20">
            <div className="w-full flex items-center justify-between bg-black/40 border border-white/5 hover:border-white/10 focus-within:border-white/20/40 rounded-2xl px-5 py-3 transition-all duration-300 shadow-inner">
              <input
                type="text"
        ...
        {/* Inner page content */}
        <motion.div variants={itemVariants} className="w-full relative px-4 lg:px-8 max-w-[1440px] mx-auto">
          <div className="relative z-10 flex flex-col gap-8 w-full mt-4">
            ...
            {/* Controls / Filter Bar */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mt-4 relative z-30 border-b border-white/5 pb-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
              ...
              <div className="lg:col-span-8 relative rounded-[32px] overflow-hidden bg-black/45 backdrop-blur-2xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-white/5 hover:border-white/10 transition-all duration-500 w-full group shadow-2xl">
              ...
              <div className="lg:col-span-4 flex flex-col justify-between gap-6">
                {/* Burnout Risk Dial */}
                <div className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 flex items-center gap-4 hover:border-white/10 transition-all duration-500 cursor-pointer shadow-lg flex-1">
                ...
                <div className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 flex items-start gap-4 hover:border-white/10 transition-all duration-500 cursor-pointer shadow-lg flex-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
              ...
              <div className="lg:col-span-8 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div
                    ...
                    className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 flex flex-col justify-between h-[280px] hover:border-white/20/25 hover:-translate-y-[2px] transition-all duration-500 group cursor-pointer shadow-2xl"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div
                    ...
                    className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 h-[280px] flex flex-col justify-between relative overflow-hidden group hover:border-white/20/25 hover:-translate-y-[2px] transition-all duration-500 cursor-pointer shadow-2xl"
                  >
              ...
              <div className="lg:col-span-4 flex flex-col gap-6 h-full">
                ...
                <div
                  onClick={() => setExplainMetric("stress")}
                  className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] overflow-hidden flex flex-col justify-between flex-1 relative group hover:border-white/20/25 hover:-translate-y-[2px] transition-all duration-500 cursor-pointer shadow-2xl"
                >
                  <div
                ...
                <div className="bg-black/45 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 space-y-4 hover:border-white/20/25 transition-all duration-500 shadow-2xl">
